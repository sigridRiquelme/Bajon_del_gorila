  ...
  // validacion final al intentar enviar el formulario (submit)
    ...
    form.addEventListener("submit", (e) => {
        ...
        if (!msgCorreo && !msgPassword) {
            // mensaje de exito
            alertSuccess.textContent = `¡Acceso concedido! Redirigiendo...`;
            alertSuccess.style.display = "block";

            // guardar simulacion de sesion
            localStorage.setItem("usuarioLogueado", correoInput.value.trim());

            // redirigir al inicio o catalogo despues de 1 sg
            setTimeout(() => {
