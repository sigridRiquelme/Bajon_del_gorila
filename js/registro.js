document.addEventListener("DOMContentLoaded", () => {
  ...
  function validarCorreo(valor) {
    ...
    if (v.length > 100) return "El correo no puede tener más de 100 caracteres.";
  ...
  function validarConfirmPassword(valor, passOriginal) {
    ...
    if (v !== passOriginal.trim()) return "Las contraseñas no coinciden.";
    return "";
  }
  ...
  function validarTelefono(valor) {
    const v = valor.trim();
    if (v === "") return "El teléfono de contacto es obligatorio.";
    const regexTel = /^[0-9+ ]{8,12}$/;
    if (!regexTel.test(v)) return "Formato inválido (ej: +56912345678 o 912345678).";
    return "";
  }
